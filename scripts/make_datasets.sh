...
TILE_SIZE=0 # Setting TILE_SIZE=0 generates a coco dataset with the full scenes, instead of tiling them first

# # dataset with vv imagery + aux
ceruleanml make-coco-dataset /root/data/partitions/test_scenes.txt /root/data/aux_datasets /root/partitions/test_tiles_context_$TILE_SIZE/ $TILE_SIZE
gsutil -m rsync -r /root/partitions/test_tiles_context_$TILE_SIZE/ gs://ceruleanml/partitions/test_tiles_context_$TILE_SIZE/

ceruleanml make-coco-dataset /root/data/partitions/val_scenes.txt /root/data/aux_datasets /root/partitions/val_tiles_context_$TILE_SIZE/ $TILE_SIZE
gsutil -m rsync -r /root/partitions/val_tiles_context_$TILE_SIZE/ gs://ceruleanml/partitions/val_tiles_context_$TILE_SIZE/

ceruleanml make-coco-dataset /root/data/partitions/train_scenes.txt /root/data/aux_datasets /root/partitions/train_tiles_context_$TILE_SIZE/ $TILE_SIZE
